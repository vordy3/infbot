...
def start(message):
    ...
    reshot=types.KeyboardButton('Ответы на задачи')
    sp=types.KeyboardButton('Справочник')
    markup.add(resh,reshot,sp)
...
def bot_message(message):
    ...
    if message.text=='Задачи':
    ...
    elif message.text=='Ответы на задачи':

        markup4=types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        num10=types.KeyboardButton('Ответ на номер 1️⃣0️⃣')
        num11=types.KeyboardButton('Ответ на номер 1️⃣1️⃣')
        num12=types.KeyboardButton('Ответ на номер 1️⃣2️⃣')
        num13=types.KeyboardButton('Ответ на номер 1️⃣3️⃣')
        num14=types.KeyboardButton('Ответ на номер 1️⃣4️⃣')
        num15=types.KeyboardButton('Ответ на номер 1️⃣5️⃣')
        bk=types.KeyboardButton('⏩Назад')

        markup4.add(num10,num11,num12,num13,num14,num15,bk)

        bot.send_message(message.chat.id,'<b>Выбери номер, который хочешь разобрать.</b>',parse_mode='html',reply_markup=markup4)

    elif message.text=='Ответ на номер 1️⃣0️⃣':
        bot.send_message(message.chat.id,'<b>Воспользуемся поисковыми средствами текстового редактора. В строке поиска введем слово «свет». Подсчитав общее количество результатов, получаем ответ</b>',parse_mode='html')
        bot.send_message(message.chat.id,'Правильный ответ: <u>4</u>',parse_mode='html')
    
    elif message.text=='Ответ на номер 1️⃣1️⃣':
        bot.send_photo(message.chat.id,answer11)
        bot.send_message(message.chat.id,'<b>Правильный ответ:</b> <u>13 байт</u>',parse_mode='html')

    elif message.text=='Ответ на номер 1️⃣2️⃣':
        bot.send_message(message.chat.id,'<b>Держи решение на питоне:</b>',parse_mode='html')
        bot.send_photo(message.chat.id,answer12)

    elif message.text=='Ответ на номер 1️⃣3️⃣':
        bot.send_message(message.chat.id,'<b>Держи решение:</b>',parse_mode='html')
        bot.send_photo(message.chat.id,answer13)

    elif message.text=='Ответ на номер 1️⃣4️⃣':
        bot.send_message(message.chat.id,'<b>Держи решение на питоне:</b>',parse_mode='html')
        bot.send_photo(message.chat.id,answer14)
        bot.send_message(message.chat.id,'<u>Ответ: 30</u>',parse_mode='html')

    elif message.text=='Ответ на номер 1️⃣5️⃣':
        bot.send_message(message.chat.id,'<b>Держи решение:</b>',parse_mode='html')
        ...
        sp=types.KeyboardButton('Справочник')
        markup.add(resh,reshot,sp)
        bot.send_message(message.chat.id,'Выбери <u>Задачи</u>, <u>Ответы</u> на задачи или <u>Справочник</u>', parse_mode='html',reply_markup=markup)
